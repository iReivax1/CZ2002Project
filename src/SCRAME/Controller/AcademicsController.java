...
public class AcademicsController {
	...
	public void passSubComponent(String subCompName, int subCompWeight, String facultyName, String courseCode) {
		for(Faculty f: facultyList) {
			if((f.getFacultyName()).equals(facultyName)) {
				if(subCompWeight > f.getCourse(courseCode).getCourseWeightage())
	...
	public Map<String, ArrayList<String>> getCourseInfoList(String facultyName)
	{
		...
		for(Course c : getFaculty(facultyName).getCourseList())
		{
			...
			System.out.println(c.getCoordinator());
	...
	public boolean addStudentToCourse(String facultyName, String courseCode, int groupNumber, boolean haveLab, String matricNumber)
	{
		...
		Student tempStud = tempCourse.getStudent(matricNumber);
	...
	public Map<String, Map<String, Double>> getCourseStats_gradePercentage(String facultyName, String courseCode)
	{
		...
		ArrayList<Results> resultList = this.getFaculty(facultyName).getCourse(courseCode).getResultList();
		...
		for(int j=0; j<9; j++)
		{
			...
			overall_stats.put(gradeList[j], overall_stats.get(gradeList[j])/this.getFaculty(facultyName).getCourseList().size());
			exam_stats.put(gradeList[j], exam_stats.get(gradeList[j])/this.getFaculty(facultyName).getCourseList().size());
			coursework_stats.put(gradeList[j], coursework_stats.get(gradeList[j])/this.getFaculty(facultyName).getCourseList().size());
		}
